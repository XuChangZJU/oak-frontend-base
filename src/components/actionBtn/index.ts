...
export default OakComponent({
    ...
    methods: {
        makeItems(isMobile?: boolean) {
            ...
            const { actions, extraActions, onAction, entity, cascadeActions } =
                this.props;
            let column = 2;
            if (schema) {
                ...
                if (extraActions?.length || actions?.length) {
                    ...
                    cascadeActions &&
                        Object.keys(cascadeActions).map(
                            (key, index: number) => {
                                ...
                                if (
                                    ...
                                    cascadeActionArr.forEach(
                                        (
                                            ele: ActionDef<ED, string | number>
                                        ) => {
                                            items.push({
                                                action:
                                                    typeof ele !== 'string'
        ...
        getLabel(actionItem: ActionDef<ED, keyof ED>, entity: keyof ED) {
            if (typeof actionItem !== 'string') {
                ...
                if (
                    ['update', 'create', 'detail', 'remove'].includes(
                        actionItem
                    )
                ) {
        ...
        getLabel2(
            ...
            path: string,
            actionItem: ActionDef<ED, keyof ED>,
            entity: keyof ED
        ) {
            if (typeof actionItem !== 'string') {
                return actionItem.label!;
