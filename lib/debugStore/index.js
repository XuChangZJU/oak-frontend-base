...
function createDebugStore(storageSchema, createContext, triggers, checkers, initialData) {
    const data = getMaterializedData();
    const executor = new TriggerExecutor_1.TriggerExecutor();
    const store = new debugStore_1.DebugStore(executor, storageSchema, data && data.data, data && data.stat);
