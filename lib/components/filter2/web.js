...
function Render(props) {
    ...
    if (isCommonI18n) {
        ...
        label = entityI18n && attrI18n ? t("".concat(entityI18n, ":attr.").concat(attrI18n)) : '';
    }
    var V;
    if (column.render) {
    ...
    switch (viewType) {
        ...
        case 'Select': {
            ...
            var isMultiple_1 = ['$in', '$nin'].includes(op || '');
            V = ((0, jsx_runtime_1.jsx)(antd_1.Select, { mode: isMultiple_1 ? 'multiple' : undefined, allowClear: true, placeholder: placeholder || t('placeholder.select'), onChange: function (value) {
                    setFilterAndResetFilter(viewType, value ? (isMultiple_1 ? value : [value]) : []);
                }, options: options2 }));
            break;
        }
        case 'DatePicker': {
