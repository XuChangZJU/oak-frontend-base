...
exports.default = OakComponent({
    ...
    formData: function () {
        var _a = this.props, multiple = _a.multiple, entityIds = _a.entityIds, pickerRender = _a.pickerRender;
        var _b = pickerRender, entity = _b.entity, projection = _b.projection, title = _b.title;
        var rows = entityIds &&
            entityIds.length &&
            this.features.cache.get(entity, {
                data: typeof projection === 'function'
                    ? projection()
                    : projection,
                filter: {
                    id: {
        ...
        var renderValue = rows && rows.length ? rows.map(function (row) { return title(row); }).join(',') : '';
        return {
