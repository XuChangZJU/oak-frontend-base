...
function reRender(option, extra) {
    ...
    if (this.state.oakEntity && this.state.oakFullpath) {
        ...
        if (actions && actions.length > 0) {
            (0, assert_1.assert)(!option.isList, 'actions目前只能作用于单个对象页面上');
            ...
            if (id_1 && value) {
                ...
                var testResult = actions.map(function (ele) { return ({
                    action: ele,
                    result: _this.checkOperation(_this.state.oakEntity, ele, undefined, { id: id_1 }, ['relation', 'row', 'logical', 'logicalRelation']),
                }); });
        ...
        var cascadeEntities = (option.cascadeEntities && option.cascadeEntities.call(this));
        if (cascadeEntities) {
            (0, assert_1.assert)(!option.isList, 'actions目前只能作用于单个对象页面上');
            ...
            if (id && value) {
                // 计算所有一对多对象的权限是否存在
                var oakCascadeEntities = {};
                var _loop_3 = function (attr) {
                    var _b, _c;
                    var rel = (0, relation_1.judgeRelation)(features.cache.cacheStore.getSchema(), this_1.state.oakEntity, attr);
                    (0, assert_1.assert)(rel instanceof Array, "".concat(this_1.state.oakFullpath, "\u4E2D\u5B9A\u4E49\u7684cascadeEntities\u7684\u952E\u503C").concat(attr, "\u4E0D\u662F\u4E00\u5BF9\u591A\u7684\u5BF9\u8C61"));
                    var _d = tslib_1.__read(rel, 2), e2 = _d[0], fkey = _d[1];
                    var strictFilter = fkey ? (_b = {},
                        _b[fkey] = id,
                        _b) : {
                        entity: this_1.state.oakEntity,
                        entityId: id,
                    };
                    var availableActions = [];
                    cascadeEntities[attr].forEach(function (item) {
                    ...
                    if (availableActions.length > 0) {
                        Object.assign(oakCascadeEntities, (_c = {},
                            _c[attr] = availableActions,
                            _c));
                    }
                };
                var this_1 = this;
                for (var attr in cascadeEntities) {
                    _loop_3(attr);
                }
                Object.assign(data, {
                    oakCascadeEntities: oakCascadeEntities,
                });
            }
        }
        if (option.isList) {
