    ...
    if (cascadeActionDict) {
        ...
        var _loop_4 = function (e) {
            ...
            if (cascadeActions) {
                ...
                var _loop_5 = function (action) {
                    ...
                    if (rows instanceof Array) {
                        if (action === 'create' || typeof action === 'object' && action.action === 'create') {
                            ...
                            var intrinsticFilter = rel_1[1] ? (_g = {},
                                _g[rel_1[1].slice(0, rel_1[1].length - 2)] = this_2.features.runningTree.getIntrinsticFilters(this_2.state.oakFullpath),
                                _g) : (_h = {},
                                _h[this_2.state.oakEntity] = this_2.features.runningTree.getIntrinsticFilters(this_2.state.oakFullpath),
                                _h);
                        ...
                        if (action === 'create' || typeof action === 'object' && action.action === 'create') {
                            var intrinsticData = rel_1[1] ? (_j = {},
                                _j[rel_1[1]] = rows.id,
                                _j) : { entity: this_2.state.oakEntity, entityId: rows.id };
                            if (typeof action === 'object') {
                                Object.assign(intrinsticData, action.data);
                            }
                            ...
                            var a2 = typeof action === 'object' ? action.action : action;
                            var filter = typeof action === 'object' ? action.filter : undefined;
                            var intrinsticFilter = rel_1[1] ? (_k = {},
                                _k[rel_1[1]] = rows.id,
                ...
                try {
                    for (var _e = (e_4 = void 0, tslib_1.__values(cascadeActions)), _f = _e.next(); !_f.done; _f = _e.next()) {
                        var action = _f.value;
                        _loop_5(action);
                    }
                }
