    ...
    if (actions) {
        var _loop_3 = function (action) {
            if (rows instanceof Array) {
                ...
                var intrinsticData = this_1.features.runningTree.getIntrinsticData(this_1.state.oakFullpath);
                if (action === 'create' || typeof action === 'object' && action.action === 'create') {
                    // 创建对象的判定不落在具体行上，但要考虑list上外键相关属性的限制
                    var data = typeof action === 'object' && action.action === 'create' ? Object.assign({}, intrinsticData, action.data) : intrinsticData;
                    if (this_1.checkOperation(this_1.state.oakEntity, 'create', data, undefined, checkTypes)) {
                    ...
                    if (this_1.checkOperation(this_1.state.oakEntity, a2_1, undefined, intrinsticData, checkTypes)) {
                ...
                (0, assert_1.assert)(!option.isList);
                (0, assert_1.assert)(!(action === 'create' || typeof action === 'object' && action.action === 'create'), 'create的action只应该定义在list页面上');
                var id = rows.id;
                var a2 = typeof action === 'object' ? action.action : action;
                var data = typeof action === 'object' ? action.data : undefined;
                if (this_1.checkOperation(this_1.state.oakEntity, a2, data, { id: id }, checkTypes)) {
                    legalActions.push(action);
    ...
    if (cascadeActionDict) {
        ...
        var _loop_4 = function (e) {
            ...
            if (cascadeActions) {
                ...
                var _loop_5 = function (action) {
                    ...
                    if (rows instanceof Array) {
                        if (action === 'create' || typeof action === 'object' && action.action === 'create') {
                            ...
                            var intrinsticFilter = rel_1[1] ? (_e = {},
                                _e[rel_1[1].slice(0, rel_1[1].length - 2)] = this_2.features.runningTree.getIntrinsticData(this_2.state.oakFullpath),
                                _e) : (_f = {},
                                _f[this_2.state.oakEntity] = this_2.features.runningTree.getIntrinsticData(this_2.state.oakFullpath),
                                _f);
                        ...
                        if (action === 'create' || typeof action === 'object' && action.action === 'create') {
                            var intrinsticData = rel_1[1] ? (_g = {},
                                _g[rel_1[1]] = rows.id,
                                _g) : { entity: this_2.state.oakEntity, entityId: rows.id };
                            if (typeof action === 'object') {
                                Object.assign(intrinsticData, action.data);
                            }
                            ...
                            var intrinsticFilter = rel_1[1] ? (_h = {},
                                _h[rel_1[1]] = rows.id,
                                _h) : { entity: this_2.state.oakEntity, entityId: rows.id };
                            var filter2 = (0, filter_1.combineFilters)([filter, intrinsticFilter]);
                            // 先尝试整体测试是否通过，再测试每一行
                            // todo，这里似乎还能优化，这些行一次性进行测试比单独测试的性能要高
                ...
                try {
                    for (var _c = (e_4 = void 0, tslib_1.__values(cascadeActions)), _d = _c.next(); !_d.done; _d = _c.next()) {
                        var action = _d.value;
                        _loop_5(action);
                    }
                }
