...
class Navigator extends Feature_1.Feature {
    ...
    async navigateTo(options, state, disableNamespace) {
        ...
        const pushAction = native_1.StackActions.push(url2, Object.assign({
            oakFrom,
        }, rest, state));
    ...
    async redirectTo(options, state, disableNamespace) {
        ...
        const url2 = this.constructUrl(url, undefined, disableNamespace);
        const oakFrom = this.getCurrentUrl();
        const replaceAction = native_1.StackActions.replace(url2, Object.assign({
            oakFrom,
        }, rest, state));
    ...
    async switchTab(options, state, disableNamespace) {
        ...
        const url2 = this.constructUrl(url, undefined, disableNamespace);
        const oakFrom = this.getCurrentUrl();
        const jumpToAction = native_1.TabActions.jumpTo(url2, Object.assign({
            oakFrom,
        }, rest, state));
