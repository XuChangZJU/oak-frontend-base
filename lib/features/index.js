...
function initializeStep2(features, aspectWrapper, storageSchema, frontendContextBuilder, checkers, authDeduceRelationMap, colorDict, getFullDataFn, getSubscribePointFn, makeBridgeUrlFn, selectFreeEntities, updateFreeDict, savedEntities, keepFreshPeriod, attrUpdateMatrix) {
    const { localStorage, environment, message } = features;
    const cache = new cache_1.Cache(storageSchema, aspectWrapper, frontendContextBuilder, checkers, getFullDataFn, localStorage, savedEntities, keepFreshPeriod, attrUpdateMatrix);
    const relationAuth = new relationAuth_1.RelationAuth(cache, authDeduceRelationMap, selectFreeEntities, updateFreeDict);
    ...
    const contextMenuFactory = new contextMenuFactory_1.ContextMenuFactory(cache, relationAuth);
