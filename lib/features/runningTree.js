...
class ListNode extends Node {
    ...
    getFreshValue() {
        const value = this.children.map(ele => ele.getFreshValue()).concat(this.newBorn.map(ele => ele.getFreshValue())).filter(ele => !!ele);
        if (this.isDirty()) {
    ...
    async composeOperation(action, execute) {
        ...
        for (const node of this.newBorn) {
            const subAction = await node.composeOperation(undefined, execute);
            if (subAction) {
                (0, assert_1.default)(!action || action === 'update'); // 如果还有新建，应该不会有其它类型的action
                actions.push(subAction);
            }
        }
        return actions;
    ...
    toggleChild(data, checked) {
        ...
        if (checked) {
            ...
            (0, assert_1.default)(false, 'toggle动作的remove对象没有找到对应的子结点');
        }
    }
}
class SingleNode extends Node {
    ...
    constructor(entity, schema, cache, projection, projectionShape, parent, action, updateData) {
        ...
        this.children = {};
        const ownKeys = [];
        Object.keys(projectionShape).forEach((attr) => {
    ...
    async composeOperation(action2, execute) {
        ...
        const operation = action === 'create' ? {
            action: 'create',
            data: Object.assign({}, this.updateData, { id: execute ? await generateNewId() : (0, mockId_1.generateMockId)() }),
        } : {
            action,
            data: (0, lodash_1.cloneDeep)(this.updateData),
            filter: {
...
class RunningTree extends Feature_1.Feature {
    ...
    schema;
    root;
    constructor(aspectWrapper, cache, schema) {
        super(aspectWrapper);
        this.cache = cache;
        this.schema = schema;
        this.root = {};
    ...
    async setUpdateDataInner(path, attr, value) {
        ...
        for (const split of attrSplit) {
            if (node instanceof ListNode) {
                ...
                (0, assert_1.default)(typeof idx2 === 'number');
    ...
    toggleNode(path, nodeData, checked) {
        const node = this.findNode(path);
        (0, assert_1.default)(node instanceof ListNode);
        node.toggleChild({
            updateData: nodeData,
