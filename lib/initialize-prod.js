 ...
 * @returns
 */
function initialize(storageSchema, createFeatures, contextBuilder, serverUrl, makeException, checkers, actionDict) {
    ...
    const wrapper = {
        exec: async (name, params) => {
            ...
            const { error, result, opRecords } = await response.json();
            if (error) {
